...
const TALLER_COLOR: Record<string, { bar: string; soft: string; text: string }> = {
  "Taller Termopaneles": { bar: "bg-indigo-500", soft: "bg-indigo-200", text: "text-indigo-600" },
  "Taller PVC (Ventanas y Puertas)": { bar: "bg-teal-500", soft: "bg-teal-200", text: "text-teal-600" },
};
...
export default async function ReportesPage({
  ...
  const r = buildReport(result.orders, filters);
  ...
  return (
    <div className="space-y-8 pb-12">
      <header className="flex flex-col items-start justify-between gap-4 md:flex-row md:items-center">
        <div>
          <h1 className="text-3xl font-extrabold tracking-tight text-slate-800">Reporte · {r.titulo}</h1>
          <p className="mt-1 text-sm text-slate-500">Resumen de ventas, fabricación y cobranza desde Odoo.</p>
        </div>
        <ReportControls filters={filters} clientes={r.clientes} />
      </header>
      ...
      <section className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="flex flex-col justify-between rounded-2xl border border-slate-100 bg-white p-6 shadow-sm lg:col-span-2">
          ...
          <div className="mt-8 space-y-6">
            {r.talleres.map((t) => {
              const c = TALLER_COLOR[t.nombre] ?? TALLER_DEFAULT;
              return (
                <div key={t.nombre}>
                  <div className="mb-2 flex items-center justify-between gap-2">
                    ...
                    <span className={`font-mono text-sm font-bold ${c.text}`}>
                      {t.terminadas}/{t.mos} MO · {t.unidades.toLocaleString("es-CL", { maximumFractionDigits: 2 })} cant.
                    </span>
                  </div>
