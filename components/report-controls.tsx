...
export function ReportControls({ filters, clientes }: { filters: ReportFilters; clientes: { id: number; name: string }[] }) {
  ...
  const go = (next: Partial<ReportFilters>) => {
    ...
    startTransition(() => router.push(`/reportes?${p}`));
  };

  return (
    <div className="flex w-full flex-col items-center gap-3 self-stretch sm:flex-row md:w-auto md:self-auto print:hidden">
